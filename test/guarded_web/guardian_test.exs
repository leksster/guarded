defmodule GuardedWeb.GuardianTest do
  use Guarded.DataCase

  alias Guarded.Accounts
  alias GuardedWeb.Guardian
  ...
  describe("#resource_from_claims for 'User:<invalid_id>' given") do
    test "returns {:error, :invalid_id} tuple" do
      ...
      assert Guardian.resource_from_claims(%{"sub" => "User:123fsa"}) == {:error, :invalid_id}
      assert Guardian.resource_from_claims(%{"sub" => "User:"}) == {:error, :invalid_id}
    end
  end

  describe("#resource_from_claims for 'User:<nion_existing>' given") do
